...
//        UpdaterHelper::get_feature_jacobian_kf()
        ...
        S.diagonal() += Eigen::VectorXd::Ones(S.rows()); //HPH^T+R
...
bool UpdaterMSCKF:: update_skf_with_KF3(State *state, std::vector<Feature *> &feature_vec, bool iterative) {
    ...
    for(size_t i=0; i<feature_vec.size(); i++) {
        ...
        int feature_cam_id = feature_vec.at(i)->querry_cam_id;
        assert(feature_vec.at(i)->uvs[feature_cam_id].size()>0);
    ...
    printf(GREEN "max_meas_size is %d\n" RESET,max_meas_size);
    ...
    while(it2 != feature_vec.end()) {
        ...
        feat.querry_cam_id = (*it2)->querry_cam_id;
        ...
        //   success = UpdaterHelper::get_feature_jacobian_kf5_multi_map(state,feat,H_f_n,H_n,H_x,res_n,Hn_order,Hx_order);
            success = UpdaterHelper::get_feature_jacobian_kf5_multi_map_norm(state,feat,H_f_n,H_n,H_x,res_n,Hn_order,Hx_order);
   ...
   UpdaterHelper::measurement_compress_inplace_with_nuisance(Hx_big,Hn_big,res_big);
    ...
    if(state->iter==false)
    {
        if((avg_error_1>state->_options.opt_thred||failed_rate>0.1)&&state->_options.opt_thred<1000)
        {
            state->iter=true;
            state->iter_count=0;
            iterative=true;
