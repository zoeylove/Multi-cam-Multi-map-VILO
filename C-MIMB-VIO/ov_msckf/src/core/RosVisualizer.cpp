...
void RosVisualizer::publish_state() {
    ...
    if(state->set_transforms[0])
    {
        R_ViotoMap= state->transform_vio_to_maps[0]->Rot();
        t_VioinMap= state->transform_vio_to_maps[0]->pos();
        R_MaptoVio = R_ViotoMap.transpose();
        t_MapinVio = - R_MaptoVio * t_VioinMap; 
        auto q_ViotoMap = state->transform_vio_to_maps[0]->quat();
