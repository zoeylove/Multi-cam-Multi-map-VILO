...
VioManager::VioManager(VioManagerOptions& params_,Simulator* sim): simulator(sim){
    ...
    else
    {
        ...
        // 创建线程并传递参数，使用 lambda 捕获外部参数
        std::thread th_matchKF([=]() {
            matchKF->DetectAndMatch(DetectAndMatch_URL, DetectAndMatch_img_save_path);  // 使用 lambda 捕获外部参数，并调用成员函数
        });
...
void VioManager::mv_do_feature_propagate_update(double timestamp, Keyframe* cur_kf_0, Keyframe* cur_kf_1, \
    ...
    //包含当前帧没有追踪上的点， 最老帧观测到的点，以及那些track轨迹足够长本可以作为slam_feature但是由于空间限制没有加入到slam_feature中的点
    ...
    if(params.use_prior_map)
    {
        ...
        if(params.use_stereo && cur_kf_0!= nullptr && cur_kf_1!= nullptr && cur_kf_2!= nullptr && cur_kf_3!= nullptr)
        {
            ...
            }else
            {
              ...
              {
                std::queue<double> temp(online_selected_kf_ts); // 复制原队列到临时队列
                int count = 0 ;
                bool check_result = 0;
      ...
      while (getline(f1,str))
      {
          ...
          else if(line_num%2==1)
          {
            ...
            if(kf==nullptr)
            {
                ...
                // //yyl1014
                // if (match_cam_id == 0 )
                //     intrinsics<<1208.29,1249.26,746.60000000000002,516.70000000000005,-0.049567815049682294,0.069208648438352491,-0.000048199233985591757,-0.001760,0;
                // else if(match_cam_id == 1)
                //     intrinsics<<1208.05,1248.73,783.60000000000002,557,-0.047353881275324715,0.065058204642788983,0.00016046245482810492,0.0021778853984350918,0;
                // else if(match_cam_id == 2)
                //     intrinsics<<500.399,516.84299999999996,507.39499999999998,407.89999999999998,-0.16624276882002614,0.040483275079251431,-0.00059364969933710778,-0.0012091693281652742,0;
                // else if(match_cam_id == 3)
                //     intrinsics<<501.88400000000001,518.01300000000003,531.59699999999998,397.49200000000002,-0.17181846847240198,0.044251920178279343,0.000085455755388265042,0.0006121597092667994,0;
                // //yylgdmap1014
                // if (match_cam_id == 0)
                //     intrinsics<<1208.2858383118951,1249.2607771622381,746.60000000000002,516.70000000000005,-0.050977770344565264,0.071225358403198505,-0.00031821463570609268,-0.0016718500279189609,0;
                // else if(match_cam_id==1)
                //     intrinsics<<1208.0540785829467,1248.7301676501809,783.60000000000002,557,-0.048602602313396248,0.066658173292657469,-0.00014368389355994395,0.0022686520184663091,0;
                // else if(match_cam_id==2)
                //     intrinsics<<500.39880432515827,516.84287412385254,507.3954,407.90019999999998,-0.17042112327442968,0.042803905784907691,-0.00039345241589052078,-0.0010742217826246495,0;
                // else if(match_cam_id==3)
                //     intrinsics<<501.88403604483852,518.01283692796221,531.59699999999998,397.49180000000001,-0.17368018089613863,0.045209090729242389,2.4230509927815519e-05,0.0007368628277891479,0;
                // //qsdjt0924
                // if (match_cam_id == 0 )
                //     intrinsics << 1197.4245076602799,1197.64728037485,744.81081704746805,520.48997517205203,-0.060748258668792902,0.047119578289576997,0.0,0.0,0;
                // else if(match_cam_id == 1)
                //     intrinsics << 1198.80717262795, 1198.6959083459799, 508.155232843075, 407.00673933076001, -0.20206914564483899, 0.064523021054268603, 0.0, 0.0,0;
                // else if(match_cam_id == 2)
                //     intrinsics << 492.73780938544098,492.43241022803102,508.155232843075,407.00673933076001,-0.20206914564483899,0.064523021054268603,0.0,0.0,0;
                // else if(match_cam_id == 3)
                //     intrinsics << 500.48965782960698,500.09060304284998,527.18325726532203,401.30960465432901,-0.20400570094544401,0.066759268211543696,0.0,0.0,0;
                // // qsdjt gdmap
                // if (match_cam_id == 0 )
                //     intrinsics << 1197.4245076602799,1197.64728037485,744.81081704746805,520.48997517205203,-0.060748258668792902,0.047119578289576997,0,0,0;
                // else if(match_cam_id == 1)
                //     intrinsics << 1198.80717262795,1198.6959083459799,508.155232843075,407.00673933076001,-0.20206914564483899,0.064523021054268603,0,0,0;
                // else if(match_cam_id == 2)
                //     intrinsics << 492.73780938544098,492.43241022803102,508.155232843075,407.00673933076001,-0.20206914564483899,0.064523021054268603,0,0,0;
                // else if(match_cam_id == 3)
                //     intrinsics << 500.48965782960698,500.09060304284998,527.18325726532203,401.30960465432901,-0.20400570094544401,0.066759268211543696,0,0,0;
                // // human map
                // if (match_cam_id == 0 )
                //     intrinsics << 683.45093688203349,682.78048500138686,584.69686364906204,350.19051150982398,0.018588806348386958,-0.034211653364466792,-0.0011166781242831069,0.00080359416381110273,0;
                // else if(match_cam_id == 1)
                //     intrinsics << 683.05129788688828,682.45983341604756,578.20448894587696,352.49287995410901,0.017365786943305265,-0.032675550390976221,-0.00022868161032691119,0.00058247485204209924,0;
                // else if(match_cam_id == 2)
                //     intrinsics << 685.95061276311117,685.35870180400536,574.42887672129496,355.20951100034699,0.017170893717012168,-0.033372222193569279,-0.00021448501980497945,0.00031350590202922849,0;
                // else if(match_cam_id == 3)
                //     intrinsics << 684.54531386162228,684.19952330837737,582.51108385730402,358.47332968548301,0.014798896570048495,-0.030198481287930526,-0.00099246090393739382,0.00029274946167436787,0;
                // //oxford math easy
                // if (match_cam_id == 0 )
                ...
                // // oxford quad median
                // if (match_cam_id == 0 )
                //     intrinsics << 352.779,354.60899999999998,359.03500000000003,260.54599999999999,-0.042169999999999999,-0.00413,0.0017899999999999999,-0.00063000000000000003,1;
                // else if(match_cam_id == 1)
                //     intrinsics << 352.53800000000001,354.50799999999998,363.85599999999999,287.49400000000003,-0.043139999999999998,0.001,-0.0028600000000000001,0.00068999999999999997,1;
                // else if(match_cam_id == 2)
                //     intrinsics << 353.47719774555992,352.49309892170646,359.38201028180089,246.07622578140331,-0.039320153983301791,-0.0047406970841331036,0.0015352229232181549,-0.00040472247420920071,1;
                // else if(match_cam_id == 3)
                //     intrinsics << 351.3873118634877,350.76423362594011,342.75711480789693,287.28484484361798,-0.036911398271645343,-0.0087927371412339066,0.0059729946700656763,-0.0019218526270038702,1;
                // // oxford quad median lio
                // if (match_cam_id == 0 )
                //     intrinsics << 352.779,354.60899999999998,359.03500000000003,260.54599999999999,-0.042169999999999999,-0.00413,0.0017899999999999999,-0.00063000000000000003,1;
                // else if(match_cam_id == 1)
                //     intrinsics << 352.53800000000001,354.50799999999998,363.85599999999999,287.49400000000003,-0.043139999999999998,0.001,-0.0028600000000000001,0.00068999999999999997,1;
