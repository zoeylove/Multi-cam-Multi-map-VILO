...
int main(int argc, char** argv)
{
    ...
    nh.param<std::string>("topic_camera1", topic_camera2, "/cam2/image_raw");
    nh.param<std::string>("topic_camera1", topic_camera3, "/cam3/image_raw");
    ...
    bool has_left_around = false;
    bool has_right_around = false;
    ...
    for (const rosbag::MessageInstance& m : view) {
        ...
        sensor_msgs::Image::ConstPtr s3 = m.instantiate<sensor_msgs::Image>();
        if (s3 != nullptr && m.getTopic() == topic_camera2) {
            // Get the image
            cv_bridge::CvImageConstPtr cv_ptr;
            try {
                cv_ptr = cv_bridge::toCvShare(s3, sensor_msgs::image_encodings::MONO8);
            } catch (cv_bridge::Exception &e) {
                ROS_ERROR("cv_bridge exception: %s", e.what());
                continue;
            }
            has_left_around = true;
            img2 = cv_ptr->image.clone();
        }

        sensor_msgs::Image::ConstPtr s4 = m.instantiate<sensor_msgs::Image>();
        if (s4 != nullptr && m.getTopic() == topic_camera3) {
            // Get the image
            cv_bridge::CvImageConstPtr cv_ptr;
            try {
                cv_ptr = cv_bridge::toCvShare(s4, sensor_msgs::image_encodings::MONO8);
            } catch (cv_bridge::Exception &e) {
                ROS_ERROR("cv_bridge exception: %s", e.what());
                continue;
            }
            has_right_around = true;
            img3 = cv_ptr->image.clone();
        }
        ...
        if(has_left_around && img2_buffer.rows == 0) {
            has_left_around = true;
            img2_buffer = img2.clone();
        }

        if(has_right_around && img3_buffer.rows == 0) {
            has_right_around = true;
            img3_buffer = img3.clone();
        }
        ...
        // If we are in stereo mode and have both left and right, then process
        if(max_cameras==4 && has_left && has_right&& has_left_around && has_right_around) {
            // process once we have initialized with the GT
            Eigen::Matrix<double, 17, 1> imustate;
            if(!gt_states.empty() && !sys->initialized() && DatasetReader::get_gt_state(time_buffer, imustate, gt_states)) {
                //biases are pretty bad normally, so zero them
            ...
            has_left = false;
            has_right = false;
            has_left_around = false;
            has_right_around = false;
            // move buffer forward
            time_buffer = time;
            img0_buffer = img0.clone();
            img1_buffer = img1.clone();
