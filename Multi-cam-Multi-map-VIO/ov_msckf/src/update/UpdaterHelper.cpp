...
bool UpdaterHelper::get_feature_jacobian_kf4_multi_map(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
    ...
    //TONY modify
    size_t cam_id = feature.querry_cam_id;
    assert(feature.timestamps[cam_id].size()>0);

    //add related clone_IMU
    for(int i=0;i<feature.timestamps[cam_id].size();i++)
    {
        total_meas++;
        {
            PoseJPL* clone=nullptr;
            clone=state->_clones_IMU.at(feature.timestamps[cam_id][i]);
            assert(clone!=nullptr);
            if(map_hx.find(clone) == map_hx.end()) {
                map_hx.insert({clone,total_hx});
                x_order.push_back(clone);
...
bool UpdaterHelper::get_feature_jacobian_kf5_multi_map_norm(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                             Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                             std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
    ...
    for(int i=0;i<feature.timestamps[cam_id].size();i++)
    {
        total_meas++;
        {
            PoseJPL* clone=nullptr;
            std::stringstream ss;
            ss << std::setprecision(19) << feature.timestamps[cam_id][i];
            string ts_string = ss.str();
            ts_string.at(0) = '1';
            double ts_new = stod(ts_string);
            clone=state->_clones_IMU.at(ts_new);
            assert(clone!=nullptr);
            if(map_hx.find(clone) == map_hx.end()) {
                map_hx.insert({clone,total_hx});
                x_order.push_back(clone);
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        cv::Point3f p_financhor = anchor_kf->point_3d_map.at(ts)[kf_feature_id];
        cout<<"p_financhor is: "<<p_financhor.x<<" "<<p_financhor.y<<" "<<p_financhor.z<<endl;
        Vector3d p_FinAnchorKF(double(p_financhor.x),double(p_financhor.y),double(p_financhor.z));
        cv::Point3f p_financhor_linp = anchor_kf->point_3d_linp_map.at(ts)[kf_feature_id];
        Vector3d p_FinAnchorKF_linp(double(p_financhor_linp.x),double(p_financhor_linp.y),double(p_financhor_linp.z));
        assert(p_financhor==p_financhor_linp);
        ...
        //hk
        Eigen::Vector3d p_FinW =  R_AnchorKFtoW * p_FinAnchorKF + p_AnchorKFinW;
        cout<<"p_AnchorKFinW is "<<p_AnchorKFinW.x()<<" "<<p_AnchorKFinW.y()<<" "<<p_AnchorKFinW.z()<<endl;
        Eigen::Vector3d p_FinW_linp = R_AnchorKFtoW * p_FinAnchorKF_linp + p_AnchorKFinW;
        if(!state->iter)
        {
          assert(p_FinW==p_FinW_linp);
        ...
        PoseJPL *extrinsics = state->_calib_IMUtoCAM.at(cam_id); 
        Matrix3d R_ItoC_linp = extrinsics->Rot_linp();
        Vector3d p_IinC_linp = extrinsics->pos_linp();
        Matrix3d R_ItoC = extrinsics->Rot();
        Vector3d p_IinC = extrinsics->pos();
        ...
        for(int i=0;i<feature.timestamps[cam_id].size();i++)
        {
            ...
            ts_string.at(0) = '1';
            double ts_new = stod(ts_string);
            PoseJPL *clone_Cur = state->_clones_IMU.at(ts_new);
            assert(clone_Cur!=nullptr);
            Matrix3d R_GtoI_linp = clone_Cur->Rot_linp();
            Vector3d p_IinG_linp = clone_Cur->pos_linp();
            Matrix3d R_GtoI_fej = clone_Cur->Rot_fej();
            ...
            // Distort the normalized coordinates (false=radtan, true=fisheye)
            Eigen::Matrix<double,2,1> uv_dist;
            Vec* distortion = state->_cam_intrinsics.at(feature.querry_cam_id); 
            Eigen::Matrix<double,8,1> cam_d = distortion->linp();
            cout<<"feature.querry_cam_id is "<<feature.querry_cam_id<<std::endl;
            std::cout<<"ts_string is "<<ts_string<<std::endl;
            // Calculate distortion uv and jacobian
            float uv_x = feature.uvs[cam_id][i](0);
            float uv_y = feature.uvs[cam_id][i](1);
            cv::Matx33d camera_k_OPENCV;
            camera_k_OPENCV(0, 0) = cam_d (0);
            camera_k_OPENCV(0, 1) = 0;
            camera_k_OPENCV(0, 2) = cam_d(2);
            camera_k_OPENCV(1, 0) = 0;
            camera_k_OPENCV(1, 1) = cam_d(1);
            camera_k_OPENCV(1, 2) = cam_d(3);
            camera_k_OPENCV(2, 0) = 0;
            camera_k_OPENCV(2, 1) = 0;
            camera_k_OPENCV(2, 2) = 1;
            // Distortion parameters
            cv::Vec4d camera_d_OPENCV;
            camera_d_OPENCV(0) = cam_d(4);
            camera_d_OPENCV(1) = cam_d(5);
            camera_d_OPENCV(2) = cam_d(6);
            camera_d_OPENCV(3) = cam_d(7);
            Eigen::Matrix<double,2,1> uv_norm_meas;
            if(state->_cam_intrinsics_model.at(0))
            {
                cv::Mat mat(1, 2, CV_32F);
                mat.at<float>(0, 0) = uv_x;
                mat.at<float>(0, 1) = uv_y;
                ...
                uv_norm_meas<<(double)mat.at<float>(0, 0),(double)mat.at<float>(0, 1);
            }
            else
            {
                cv::Mat mat(1, 2, CV_32F);
            ...
            // Our residual
            cout<<"measure feature: "<<uv_norm_meas.transpose()<<endl;
            cout<<"predict feature in curimage: "<<uv_norm_linp.transpose()<<endl;
            // if(uv_dist(0)<0||uv_dist(1)<0)
            // {
            //   return false;
            // }
            res.block(2*c,0,2,1) = uv_norm_meas - uv_norm_linp;
            ...
            // Precompute some matrices
            Eigen::Matrix<double,2,3> dz_dpfc = dzn_dpfc;
            Eigen::Matrix<double,2,3> dz_dpfg = dz_dpfc*dpfc_dpfg;
            Eigen::Matrix<double,2,3> dz_dpfw = dz_dpfg*dpfg_dpfw;
        ...
        // Calculate distortion uv and jacobian
        Eigen::Matrix<double,2,1> uv_m;
        float uvm_x = anchor_kf->point_2d_uv_map.at(ts)[kf_feature_id].x;
        float uvm_y = anchor_kf->point_2d_uv_map.at(ts)[kf_feature_id].y;
        cv::Matx33d camera_k_OPENCV_kf;
        camera_k_OPENCV_kf(0, 0) = kf_d(0);
        camera_k_OPENCV_kf(0, 1) = 0;
        camera_k_OPENCV_kf(0, 2) = kf_d(2);
        camera_k_OPENCV_kf(1, 0) = 0;
        camera_k_OPENCV_kf(1, 1) = kf_d(1);
        camera_k_OPENCV_kf(1, 2) = kf_d(3);
        camera_k_OPENCV_kf(2, 0) = 0;
        camera_k_OPENCV_kf(2, 1) = 0;
        camera_k_OPENCV_kf(2, 2) = 1;
        // Distortion parameters
        cv::Vec4d camera_d_OPENCV_kf;
        camera_d_OPENCV_kf(0) = kf_d(4);
        camera_d_OPENCV_kf(1) = kf_d(5);
        camera_d_OPENCV_kf(2) = kf_d(6);
        camera_d_OPENCV_kf(3) = kf_d(7);
        Eigen::Matrix<double,2,1> uv_norm_meas_kf;
        if(state->_cam_intrinsics_model.at(0))
        {
            cv::Mat mat(1, 2, CV_32F);
            mat.at<float>(0, 0) = uvm_x;
            mat.at<float>(0, 1) = uvm_y;
            mat = mat.reshape(2); // Nx1, 2-channel
            // Undistort it!
            cv::fisheye::undistortPoints(mat, mat, camera_k_OPENCV_kf, camera_d_OPENCV_kf);
            // Construct our return vector
            cv::Point2f pt_out;
            mat = mat.reshape(1); // Nx2, 1-channel
            uv_norm_meas_kf<<(double)mat.at<float>(0, 0),(double)mat.at<float>(0, 1);
        }
        else
        {
            cv::Mat mat(1, 2, CV_32F);
            ...
            mat = mat.reshape(2); // Nx1, 2-channel
            // Undistort it!
            cv::undistortPoints(mat, mat, camera_k_OPENCV_kf, camera_d_OPENCV_kf);
            // Construct our return vector
            cv::Point2f pt_out;
            mat = mat.reshape(1); // Nx2, 1-channel
            uv_norm_meas_kf<<(double)mat.at<float>(0, 0),(double)mat.at<float>(0, 1);
        ...
        cout<<"p_FinAnchorKF: "<<p_FinAnchorKF.transpose()<<endl;
        cout<<"intrinsice: "<<kf_d.transpose()<<endl;
        cout<<"measure feature: "<<uv_norm_meas_kf.transpose()<<endl;
        cout<<"predict feature in mapkf: "<<uv_norm_kf_linp.transpose()<<endl;
        res.block(2*c,0,2,1) = uv_norm_meas_kf - uv_norm_kf_linp;

        Eigen::Matrix<double,2,3> dzn_dpfk = Eigen::Matrix<double,2,3>::Zero();
        dzn_dpfk << 1/p_FinAnchorKF_linp(2),0,-p_FinAnchorKF_linp(0)/(p_FinAnchorKF_linp(2)*p_FinAnchorKF_linp(2)),
                0, 1/p_FinAnchorKF_linp(2),-p_FinAnchorKF_linp(1)/(p_FinAnchorKF_linp(2)*p_FinAnchorKF_linp(2));

        H_f.block(2*c,0,2,H_f.cols()).noalias()=dzn_dpfk;
        // Move the Jacobian and residual index forward
        c++;
...
bool UpdaterHelper::get_feature_jacobian_kf5_multi_map(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        for(int i=0;i<feature.timestamps[cam_id].size();i++)
        {
            ...
            double uv_x = feature.uvs[cam_id][i](0);
            double uv_y = feature.uvs[cam_id][i](1);
            Vector2d uv_feat(uv_x,uv_y);
...
//           sleep(1);
            ...
            uv_m << (double)keyframe->point_2d_uv_map.at(ts)[kf_feature_id].x, (double)keyframe->point_2d_uv_map.at(ts)[kf_feature_id].y;
            cout<<"measure feature: "<<uv_m.transpose()<<endl;
            cout<<"predict feature in another mapkf: "<<uv_dist_kf.transpose()<<endl;
            res.block(2*c,0,2,1) = uv_m - uv_dist_kf;
